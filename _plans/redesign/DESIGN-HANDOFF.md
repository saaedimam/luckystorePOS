# 5dbbd7eb-8f17-459e-9b3a-5dbdda7165f2 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `luckystorepos-admin.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `luckystorepos-admin.html`
...
- A single self-contained `luckystorepos-admin.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `luckystorepos-admin.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `luckystorepos-admin.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
