...
.ai/                       # AI command center
  MASTER_RULES.md         # AI Constitution (safety & security)
  AGENT_ONBOARDING.md     # Quick start guide
  AI_TASKS.md            # Active task queue
  llm_config.json        # Model routing config
  prompts/               # Reusable AI prompts
  context/                 # Per-model instructions
    CLAUDE.md
    GEMINI.md

.vibe/current/context.md  # Session state (auto-loaded)
...
### Session Protocol (Auto-Triggered)
- **`.ai/MASTER_RULES.md`** - AI Constitution (safety, security, quality rules)
- **`.ai/AI_TASKS.md`** - Active task queue and sprint status
- **`.vibe/current/context.md`** - Session state and recent actions
- **`./scripts/dev/agent-verify.sh`** - Environment compliance checker

These files are automatically loaded at session start per MANDATORY protocol above.
...
*CLAUDE.md Version: 2026.05.25-v6*
*Architecture Reference: ARCHITECTURE.md, AGENTS.md, docs/vibe-guides/*
*Session Protocol: Auto-loads MASTER_RULES.md at startup*
...
**At the start of EVERY new session, Claude MUST automatically:**

1. **Read `.ai/MASTER_RULES.md`** - AI Constitution (highest authority)
2. **Read `.ai/AI_TASKS.md`** - Active task queue
3. **Read `.vibe/current/context.md`** - Session state
4. **Run `./scripts/dev/agent-verify.sh`** - Environment compliance check
...
**MASTER_RULES.md contains safety and security rules that OVERRIDE any other instructions.**