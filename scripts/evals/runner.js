...
// scripts/evals/eval-runner.ts
var eval_runner_exports = {};
__export(eval_runner_exports, {
  EvalRunner: () => EvalRunner
});
module.exports = __toCommonJS(eval_runner_exports);
...
// scripts/evals/invariant-verifier.ts
var dotenv = __toESM(require_main4());
var import_path = require("path");
dotenv.config({ path: (0, import_path.resolve)(process.cwd(), "apps/admin_web/.env.local") });
var InvariantVerifier = class {
  supabase;
  constructor() {
  ...
  async verifyLedgerSums() {
    ...
    const { data: stocks, error: stockErr } = await this.supabase.from("stock_levels").select("store_id, item_id, qty_on_hand");
    if (stockErr) throw stockErr;
    let anomalies = 0;
    let totalChecks = 0;
    const drifts = [];
    let overallAuthority = "AUTHORITATIVE" /* AUTHORITATIVE */;
    for (const stock of stocks || []) {
      totalChecks++;
      const { data: movements, error: movErr } = await this.supabase.from("inventory_movements").select("quantity_delta, new_quantity, operation_id").eq("store_id", stock.store_id).eq("item_id", stock.item_id).order("created_at", { ascending: false });
  ...
  async verifyAppendOnlyMath() {
    ...
    const { data: movements, error } = await this.supabase.from("inventory_movements").select("id, store_id, item_id, previous_quantity, quantity_delta, new_quantity").order("item_id", { ascending: true });
    ...
    return {
      ...
      // Internal math check is authoritative over its own schema context
      anomalies
    };
  }
  async runAll() {
...
// scripts/evals/eval-runner.ts
dotenv2.config({ path: (0, import_path2.resolve)(process.cwd(), "apps/admin_web/.env.local") });
var EvalRunner = class {
  ...
  async setup() {
    ...
    const initRes = await this.evalUserClient.rpc("set_inventory_stock", {
  ...
  async testDuplicateReplay() {
    ...
    const { data: stock } = await this.serviceClient.from("stock_levels").select("qty_on_hand").eq("store_id", this.testStoreId).eq("item_id", this.testProductId).single();
