...
const supabaseAdmin = (0, supabase_js_1.createClient)(SUPABASE_URL, SERVICE_ROLE_KEY);
...
async function main() {
    ...
    if (tErr)
        throw new Error(`Tenant setup failed: ${tErr.message}`);
    ...
    if (sErr)
        throw new Error(`Store setup failed: ${sErr.message}`);
    ...
    if (iErr)
        throw new Error(`Item setup failed: ${iErr.message}`);
    ...
    if (stErr)
        throw new Error(`Stock setup failed: ${stErr.message}`);
    ...
    const clientA = (0, supabase_js_1.createClient)(SUPABASE_URL, ANON_KEY);
