...
BEGIN
  IF EXISTS (
    ...
    WHERE proname = 'complete_sale'
      AND pg_get_function_identity_arguments(oid) = 'uuid, uuid, uuid, jsonb, jsonb, numeric, text, text'
  ) THEN
    EXECUTE 'REVOKE ALL ON FUNCTION public.complete_sale(uuid,uuid,uuid,jsonb,jsonb,numeric,text,text) FROM PUBLIC';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.complete_sale(uuid,uuid,uuid,jsonb,jsonb,numeric,text,text) TO authenticated';
  END IF;
END $$;
