...
begin
  -- Sales
  select coalesce(sum(total_sales), 0) into v_sales_now 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date);
    
  select coalesce(sum(total_sales), 0) into v_sales_prev 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date - interval '1 month');

  -- Purchases
  select coalesce(sum(stock_purchase), 0) into v_purch_now 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date);

  select coalesce(sum(stock_purchase), 0) into v_purch_prev 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date - interval '1 month');

  -- Expenses
  select coalesce(sum(daily_expense), 0) into v_exp_now 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date);

  select coalesce(sum(daily_expense), 0) into v_exp_prev 
  from daily_sales 
  where store_id = p_store_id 
    and date_trunc('month', sale_date) = date_trunc('month', current_date - interval '1 month');
